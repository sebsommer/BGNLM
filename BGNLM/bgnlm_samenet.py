...
def var_inference(values, comps, iteration, family, net = None, prior_func_a = gamma_prior, verbose=VERBOSE, learnig_rate = LEARNIG_RATE):
    ...
    if iteration > 1:
        optimizer = optim.Adam(bayes_net.parameters(), lr=0.0001)
    else:
    ...
    for j in range(x_train.shape[1], POPULATION_SIZE):
        ...
        while (True):
            ...
            if comp <= COMPLEXITY_MAX and np.all(np.isfinite(train_vals)) and not collinear:
                ...
                break
            continue
    net = BayesianNetwork(POPULATION_SIZE + 1, alpha_prior = gamma_prior(np.array([F0[k]['complexity'] for k in range(POPULATION_SIZE)]), x_train.shape[1], POPULATION_SIZE), family=family).to(DEVICE)
...
for i in range(1, N_GENERATIONS + 1):
    ...
    mprobs, net = var_inference(train_values, complexities, i, family, net, gamma_prior)
