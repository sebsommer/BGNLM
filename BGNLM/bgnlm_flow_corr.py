...
def printF0(dict):
        ...
        if family == 'binomial':
            ...
            print('\n\nTEST RESULTS:\n----------------------')
            print('GENERATION LOSS:', np.sqrt(loss/x_test.shape[0]))
st = time.time()
...
arg_help = "{0} -d <dataset nr> -g <number of generations> -e <extra features> -c <max complexity> -v <verbose> -f <use_flows>".format(sys.argv[0])
try:
    opts, args = getopt.getopt(sys.argv[1:], "hd:g:v:e:c:y:s:f:", ["help", "dataset nr=", "generations=", "extra features=", "max complexity=", "verbose=", "y=", "sigma=", "use_flows="])
except:
    print(arg_help)
    sys.exit(2)
...
#spam (binomial)
    ...
    print("Data: Superconductor, normal. IAF layers = ", N_IAFLAYERS)
...
#
    ...
    X = np.random.normal(0,1, (100000, 20))
    beta = np.array([0,0,0,0,0, 1.5, -4, 3, -0.2, 1,0,0,0,0,0, -2, 1.3, 0.3, -0.8, 3])

    
    sigma = 1
    noise = np.random.normal(0,sigma, size = 100000)

    try: alpha
    except NameError:
        print("\nNeed to correlation level!\n")
        alpha = float(input("Provide alpha for sim study 1: "))
    ...
    family = 'gaussian'
    print("Data: sim-data 1 correlation, alpha =", alpha)
...
if dataset_nr == '1':
...
elif dataset_nr == 'sim1' or dataset_nr == 'sim1_corr':
    def gamma_prior(comps, n, p):
        return torch.from_numpy(np.exp(-2*np.log1p(comps+1)-0.000001))
    ...
    #transformations = [p05, pm05, p0pm05, pm1, p1]
    tprobs = [1./len(transformations) for _ in range(len(transformations))]
    fprobs = [1/2, 1/2]
    def gamma_prior(comps, n, p):
        return torch.from_numpy(np.exp(-(1 + np.log(2**(comps)))*np.log(2)))
    def generate_feature(pop, val, target, comps, fprobs, mprobs, tprobs, family,train_x, test_x,x_cols):
...
if plot:
    ...
    idx = y_test.argsort()
    #sdy = y_test.std()
    idx =  idx[np.random.choice(2, size=idx.shape[0],  p =[0.9, 0.1])==1]
    # fig1, ax1 = plt.subplots()
    # ax1.plot(predicted_vals[test_loss.index(min(test_loss))][idx], color = "red", label = 'predicted', linestyle = " ", marker = '.')
    # ax1.plot(upper_vals[test_loss.index(min(test_loss))][idx], color = "blue", label = '0.95 quantile' ,linestyle="dashed", marker = '.')
    # ax1.plot(lower_vals[test_loss.index(min(test_loss))][idx], color= "blue", label = '0.05 quantile' ,linestyle="dashed", marker = '.')
    ...
    with open('plotvalues_abalone.txt', 'w') as f:
        f.write(f'pred:\t {predicted_vals[test_loss.index(min(test_loss))][idx]}\n')
        f.write(f'y: \t{y_test[idx]}\n')
        f.write(f'upper: \t{upper_vals[test_loss.index(min(test_loss))][idx]}\n')
        f.write(f'lower: \t{lower_vals[test_loss.index(min(test_loss))][idx]}\n')
