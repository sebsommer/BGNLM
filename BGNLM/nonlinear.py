...
class percentile75(Non_linear):
    def __init__(self, feature):
        super().__init__(feature)
        self.comp = 1
    def __str__(self):
        return 'I(' + str(self.feature) + ' > F_75^(-1)(' + str(self.feature) + '))'
    def evaluate(self, x):
        return np.where(x >= np.percentile(x, 75), 1, 0)
    
class percentile25(Non_linear):
    def __init__(self, feature):
        super().__init__(feature)
        self.comp = 1
    def __str__(self):
        return 'I(' + str(self.feature) + ' > F_25^(-1)(' + str(self.feature) + '))'
    def evaluate(self, x):
        return np.where(x >= np.percentile(x, 25), 1, 0)