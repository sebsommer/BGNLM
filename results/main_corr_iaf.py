...
#gaussian
    ...
    mu = 0.1 + normalize(p05('x1').evaluate(X['x1'])) + normalize(X['x1']) + normalize(pm05('x3').evaluate(X['x3'])) + normalize(p0pm05('x3').evaluate(X['x3'])) + normalize(X['x4a']) + normalize(pm1('x5').evaluate(X['x5'])) + normalize(p1('x6').evaluate(X['x6'])) + normalize(X['x8']) + normalize(X['x10'])

    x_cols = X.iloc[:,2:14].columns.to_numpy()
    Y = X.iloc[:,14:]
    
    s2 = float(sigma)
    y = np.random.normal(loc = mu, scale = sigma, size = X.shape[0])
