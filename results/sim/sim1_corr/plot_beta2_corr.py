...
posterior_mean = np.array([0.14040437, 0.014760423, 0.027898928, 0.017289625, 0.0092413435, 0.0072483853, 0.007767214, 0.00047917906, 4.9379687e-05])
lower = np.array([0.12348417926579713, -0.0033098718523979186, 0.008005328848958018, -0.003519305810332298, -0.0113204151019454, -0.010024655293673277, -0.012271186392754315, -0.004619445871794596, -0.0015090104959381279])
upper = np.array([0.15732456754893065, 0.03283071875572205, 0.04779252745211124, 0.03809855677187443, 0.029803102053701878, 0.024521425981074572, 0.027805614341050385, 0.00557780398405157, 0.001607769869879121])

sns.set()
fig1, ax1 = plt.subplots()
plt.title(r'$\beta_2$')
ax1.plot(posterior_mean, color = "red", label = 'posterior mean')
ax1.plot(np.zeros(9), color = "black", label = 'truth')
ax1.plot(upper, color = "red", label = '0.95 quantile' ,linestyle="dashed", alpha = 0.5)
ax1.plot(lower, color= "red", label = '0.05 quantile' ,linestyle="dashed", alpha = 0.5)
ax1.legend()
fig1.savefig('beta2_corr.png')
print("Prediction plot saved as 'beta2_corr.png'")
plt.close(fig1)
...
beta_pred = np.zeros((1, 9))
beta_upper = np.zeros((1, 9))
beta_lower = np.zeros((1, 9))
for i, s in enumerate([100, 50, 25 ,10, 5, 4, 3, 2, 1, 0.1, 0.01, 0.001]):
    noise = np.random.RandomState(110495).normal(0,s, size = 15000)
    ...
    u = posterior_mu_beta + 1.96*np.sqrt(beta_sigma)
    l = posterior_mu_beta - 1.96*np.sqrt(beta_sigma)

    beta_pred[:,i] = posterior_mu_beta
    beta_lower[:,i] = l
    beta_upper[:,i] = u


true = np.array([0,0,0,0,0, 1.5, -4, 3, -0.2, 1,0,0,0,0,0, -2, 1.3, 0.3, -0.8, 3])

for i in range(12):
    print(list(beta_pred[:,i]))

sns.set()
plt.rcParams['figure.constrained_layout.use'] = True
fig, axes = plt.subplots(5,4, figsize=(9, 12))
for i, ax in enumerate(axes.flat):
    truth = [true[i] for _ in range(12)]
    ax.plot(truth, label = 'truth', color = 'black')
    ax.plot(beta_pred[i,:], label = rf'$\beta_{i}$', color = 'red')
    ax.plot(beta_lower[i,:], color = 'red', linestyle= 'dashed', alpha=0.5)
    ax.plot(beta_upper[i,:], color = 'red', linestyle= 'dashed', alpha=0.5)
    ax.set_title(r'$\beta_{{{}}}$'.format(i+1))
    if (i - 16) >= 0:
        plt.sca(ax)
        plt.xticks(range(12), ['100', '50', '25', '10', '5', '4', '3', '2', '1', '0.1', '0.01', '0.001'], rotation = 70, fontsize=9)
    else:
        plt.sca(ax)
        plt.xticks(range(12), ['' for _ in range(12)])
    plt.yticks(np.round([true[i]-0.5, true[i], true[i]+0.5], 1), [f'{np.round(true[i]-0.5, 1)}',f'{np.round(true[i], 1)}', f'{np.round(true[i]+0.5, 1)}'], fontsize=9)
    plt.ylim([true[i]-0.6, true[i]+0.6])
    #ax.legend()
fig.suptitle(r'$\beta$-coefficients for simulation study 1')
#fig.tight_layout()
fig.savefig('all_beta_sim1_mf.png')
print("Prediction plot saved as 'all_beta_sim1.png'")
plt.close(fig)